...
###2蓝牙低功耗说明
这一章描述了ble协议栈的不同层，这些层包含了组件，和他们的概念
####2.1 GAP层(通用接入规范)
GAP是蓝牙协议栈最底层的应用程序接口。它包含管理广播和连接等的一些参数。
注意：关于GAP的更详细的信息到 Bluetooth Core Specification 卷三的c部分查看

#####2.1.1  角色
创建和 保持BLE连接与某些角色有关。一个BLE设备是一个主设备，或者是一个从设备，定义依赖于连接的初始化。主设备总是初始化连接，然而从设备发起连接。把主和从设备分别称为主设备和从设备在他们连接的时候。  
在LED 按键应用中，NRF51822片上程序使用的是 S110 SoftDevice 将作为从设备 ，一个pc或手机将作为主设备。
另外主设备和从设备的角色，蓝牙核心规范中定义的是观察和广播角色。观察者监听在 空中发生的事件，广播角色发送但是并不接收信息。这些 应用并不适用于我们的情况。
注意：在连接的另一端的设备经常称为对等设备。即使设备在中心或者外围。  
#####2.1.2  广播
对于一个主设备来说能连接从设备，从设备必须被 广播。它每隔段时间会发送广播包，被称为广播间隔那在20ms到10.24s之间不等。广播间隔影响着它花费多长时间初始化连接。  
主设备必须接收一个广播包在它能发送一个连接请求初始化连接之前。从设备只能监听连接请求一会在发送完一个广播包之后。  
一个广播包包含数据的31个字节。它经常包含使用者能读的名字，关于设备发送包的信息，以及一些设备是否连接或没有连接的标志位等等。  
在一个主设备受到广播包的时候，它将发送一个请求为了更多的广播数据，称为一次Scan Rquest。如果它被配置作为一个Active Scanner。一个外围 设备回应请求通过发送一个扫描响应包含额外的31个字节的。  
广播，包括扫描请求和响应，发生在2.4Ghz的3个频带上避免wlan的干扰。
#####2.1.3  扫描
扫描被主设备用来监听广播包和发送扫描请求。有两个参数你需要了解在 这个上下文中：扫描窗口和扫描间隔。  
对于每个扫描间隔，主设备扫描一次等于扫描窗口，意味着如果扫描串口等于扫描间隔，主设备将做一个连续扫描。主设备的扫描周期称为扫描窗口除以扫描间隔，用百分比表示。 

#####2.1.4  初始化
在主设备箱进入一个连接的时候，它将使用相同的程序扫描监听广播包。在初始化的时候，主设备将发送一个连接请求道从设备在它收到一个广播包的时候。 
#####2.1.5  连接
定义了主设备和从设备在第一次数据交换的一次连接中。在一次连接的时候，主设备将从外围 设备请求数据在指定的时间间隔内。这个间隔被称为连接间隔。他被主设备决定和应用于连接，但是一个从设备能设置连接参数更新到主设备的连接请求。连接 间隔必须在7.5ms和4ms之间根据蓝牙核心规范。  

如果从设备没有从主设备响应包，在指定的时间帧内称作连接监控超时，连接考虑丢失。  
那是可能的获得更高的数据通过在每个连接间隔中传输多倍的包。每个包传输能达到20个字节。然而，如果当前的消耗很重要并且从设备没有数据要发送，它就能选择忽略一定数量的时间间隔。忽略的时间间隔称作从机延时。
在一个连接中，设备将调频通过所有的频带，除了广播通道，在某种程度上那对应用程序是完全透明的传输。
...
对于led，按键的例子，从设备（带有led灯和按键——作为服务器的角色，主设备作为客户端的角色
#####2.2.2 GATT的层级
...
对于LED的特征值，主设备需要能设置它的值，并且可以读它。既然应用程序级别的应答不需要，你能用不带应答的写和读属性。 
...
###LED Button应用例程 
LED按键应用例程被创建为了给你提供一个怎样使用基于nRF51822芯片蓝牙低功耗的环境。他是简单的BLE应用，演示了在BLE上的双向通信。在它运行的时候，你将能切换LED输出在nRF51822芯片上来自主设备，并且收到一个通知在按键输入被切换的时候在nRF51822上。  
